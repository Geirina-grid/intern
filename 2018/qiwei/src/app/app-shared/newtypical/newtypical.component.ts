// copied from all-tl.component.ts
...
import { NewclusteringService } from '../services/newclustering.service';
const GraphChart = (<any>gvis).GraphChart;

@Component({
  selector: 'app-newtypical',
  templateUrl: './newtypical.component.html',
  styleUrls: ['./newtypical.component.css'],
  providers: [NewclusteringService]
})

export class NewtypicalComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    console.log('newtypical ran haha...');
    this.newclusteringService.plot_all(30).then(
      res=>{
  ...
  ngOnInit() {
console.log('newtypical ran again hahaha...')
  }
  ...
  // }
  runal(){  
    this.newclusteringService.run_algo(this.nclus)
    this.newclusteringService.plot_all(this.nclus).then(
      res=>{
        let i=0;
        for (i=0;i<res.length;i++){
  ...
  onSeriesSelect(e){
    ...
    chart:{zoomType:'x',
     ...
     series:[{
             ...
             color:'black'}]};
    this.options1=opt;
    this.newclusteringService.plot_one_pie(parseInt(e.context.options.name)).then(
      res=>{
        // console.log(res);
        this.options2=res[0];
        this.userinfo=res[1];
