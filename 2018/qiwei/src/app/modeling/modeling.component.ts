...
import { GraphManageFormComponent } from '../app-shared/components/graph-manage/graph-manage-form/graph-manage-form.component';
...
export class ModelingComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  @ViewChild('container') container: any;
  @ViewChild('nodeDiag') nodeForm: ModelingFormComponent;
  @ViewChild('linkDiag') linkForm: ModelingFormComponent;
...
// (Jack) this part dealing with the MENU on graphchart (right-click on node or edge)
  ...
  // (Jack) Do we start to "define" or "use" the payload service from here?? NO!
  treeLayout(): void {
    // add root node
    console.log('tree');
  ...
  // (Jack) the first time we define the payload and used the postGraph method!
  saveLayout(): void {
    ...
    this.modelingService.postGraph(JSON.stringify({ vertices: { bus_D: payload } }))
  ...
  discardConfirm() {
    this.confirmationService.confirm({
      message: 'Discard all changes REALLY?',
      accept: () => {
        this.discardLayout();
      }
    });
  ...
  showUpdateNode() {
    ...
    if (selectedNodes[0]) {
      ...
      this.addNodeDisplay = true; // showing the same "Loadpoint Editor" as triggered by "Add Loadpoint"
    } else {
  ...
  addNode() {
    ...
    if (!formValue.type || !formValue.exId) {
      ...
      this.modelingService.postGraph(JSON.stringify(payload))
        .then(res => {
          console.log(res);
          if (!res.error) {
  ...
  deleteNode() {
    ...
    if (selectedNodes[0]) {
      ...
      this.modelingService.deleteNode(selectedNodes[0].exType, selectedNodes[0].exID)
        .then(res => {
          console.log(res);
          if (!res.error) {
            return this.wholeGraphService.getGraph();
  ...
  deleteNodeConfirm() {
    this.confirmationService.confirm({
      message: 'Delete selected Node REALLY?',
      accept: () => {
        this.deleteNode();
      }
    });
  ...
  deleteLink() {
    ...
    if (selectedLinks[0]) {
      ...
      this.modelingService.deleteLink(selectedLinks[0].source.exType, selectedLinks[0].source.exID, selectedLinks[0].exType, selectedLinks[0].target.exType, selectedLinks[0].target.exID)
        .then(res => {
          console.log(res);
          if (!res.error) {
            return this.wholeGraphService.getGraph();
