...
// import { ClusteringService } from '../../data-analysis/services/clustering.service'
import { NewclusteringService } from '../app-shared/services/newclustering.service';
...
@Component({
  selector: 'app-newclustering',
  templateUrl: './newclustering.component.html',
  styleUrls: ['./newclustering.component.css'],
  // providers: [ClusteringService, DataQualityService]
  providers: [NewclusteringService, DataQualityService]
})


export class NewclusteringComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private newclusteringservice: NewclusteringService,
    ...
    this.idSubscription = this.idService.id$.subscribe(
      (id) => {
        ...
        this.newclusteringservice.get_phase(this.selectedmeter).then(
          res => {
            this.phases = res;
  ...
  onmouseOver(e) {
    ...
    this.newclusteringservice.plot_line(this.selectedmeter, this.selectedphase, e.context.category, e.context.y + 1).then(
      res => {
  ...
  plotscat() {
    this.newclusteringservice.get_phase(this.selectedmeter).then(
      res => {
        ...
        // this.selectedphase = this.phases[0].value;
        this.newclusteringservice.plot_scatter(this.selectedmeter, this.selectedphase).then(
          sp => {
            //console.log(sp);
            if (sp == null) {
              console.log("sp1");
              ...
              this.options1 = sp[0];
              this.newclusteringservice.plot_line(this.selectedmeter, this.selectedphase, '01/01/13', sp[0].series[0].data[0].y + 1).then(
                res => {
                  res[0].chart.width = this.iW * 0.37;
                  res[0].chart.height = (this.iH - 95) * 0.6;
                  this.options2 = res[0]
