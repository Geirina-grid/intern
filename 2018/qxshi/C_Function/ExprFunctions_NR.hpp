...
inline string GainMatrix_factorize_static (int64_t &gslackbus, int64_t &gnnz_GainP, int64_t &gnnz_GainQ, ArrayAccum<SumAccum<int64_t>>& gGainP_p, ArrayAccum<SumAccum<int64_t>>& gGainQ_p, ArrayAccum<ListAccum<T>>& gGainP, ArrayAccum<ListAccum<T>>& gGainQ)
  ...
  //pivot_inv = graphlu->pivot_inv; // pivoting array  
	...
	CSRMatrix *L_GainP = SingletonMatrixInterface::getMatrixLp();
	CSRMatrix *U_GainP = SingletonMatrixInterface::getMatrixUp();
  ...
  //pivot_inv_pp = graphlu_pp->pivot_inv; // pivoting array   
	...
	std::cout << "\n ======================== Save L_GainQ and U_GainQ ======================== \n"<< std::endl;
  	// static testing [Chen Yuan]  
	CSRMatrix *L_GainQ = SingletonMatrixInterface::getMatrixLpp();
	CSRMatrix *U_GainQ = SingletonMatrixInterface::getMatrixUpp();
...
// **********************************************************************************************************************************
// Created by: Qingxin Shi, ting.chan@geirina.net
// Date: 7/12/2018
// This function takes an index and a HeapAccum as inputs and returns Zth (Thevenin equivalent) as output. Specified for gVertex
// History:
// **********************************************************************************************************************************
template <typename T_vertex_tuple, typename vertex_comp>
inline double GRAPHLU_getZth (uint64_t& index, const HeapAccum<T_vertex_tuple, vertex_comp> &gVertex) {
...
// **********************************************************************************************************************************
template <typename T_vertex_tuple, typename vertex_comp>
inline double GRAPHLU_getSTI (uint64_t& index, const HeapAccum<T_vertex_tuple, vertex_comp> &gVertex) {
...
inline void GRAPHLU_NRpf_v4 (HeapAccum<T_vertex_tuple1, vertex_comp1> &gVertex, HeapAccum<T_edge_tuple1, edge_comp1> &gEdge, MaxAccum<int> maxiter, MaxAccum<double> maxchange) {
	...
	//// Convert vectors to GRAPHLU Data Structure (pointers)
	real__t *eG, *eB, *deltaP, *deltaQ, *P_operation, *Q_operation, *b, *Vm, *Va, *dVm_sens, *dVa_sens, *Pn, *Qn, *H,*N, *J, *L, *Pcal, *Qcal, *Jx, *deltaX, *X_operation, *Zth, *ZL, *STI; 
 ...
 sine_array = (real__t *)malloc(sizeof(real__t)*(nnz));
	...
	P_operation = (real__t *)malloc(sizeof(real__t)*(n)); // b in the Ax=b. Parameter for VSA
	Q_operation = (real__t *)malloc(sizeof(real__t)*(n)); // b in the Ax=b. Parameter for VSA
	X_operation = (real__t *)malloc(sizeof(real__t)*(n_J)); // P_operation and Q_operation, a 2n x 1 array. Parameter for VSA
	...
	Vm = (real__t *)malloc(sizeof(real__t)*(n)); 
	Va = (real__t *)malloc(sizeof(real__t)*(n)); 
	dVm_sens = (real__t *)malloc(sizeof(real__t)*(n)); // Parameter for VSA
	dVa_sens = (real__t *)malloc(sizeof(real__t)*(n)); // Parameter for VSA 
	double Zth;
	double ZL;
	STI = (real__t *)malloc(sizeof(real__t)*(n)); 	// Sensitivity index for VSA
   ...
   // dVm_sens[i] = gVertex.data_[i].dVm_sens;	// Parameters for VSA
   // dVa_sens[i] = gVertex.data_[i].dVa_sens;	// Parameters for VSA
   btype[i] = gVertex.data_[i].bustype;
   std::cout << "Vm0:" << Vm[i] << " Va0:"  << Va[i] << " Pn0:" << Pn[i] << " Qn0:" << Qn[i] << std::endl;
   ...
   */    
   ...
   //GraphLU_SolveFast(graphlu, deltaX); // This is fast forward/backward, which uses sparse vector. This is fast when there are many zeros in b
	...
	for (i=0; i<n; ++i){
		...
		// store P and Q (at operation point) into the b vector	
		X_operation[2*i]=P_operation[i];
		X_operation[2*i+1]=Q_operation[i];
	}	 
	...
	// Update the Jacobian matrix using the final power flow result
	// std::cout << " ======================== Get H,N,J,L matrices ========================"<<std::endl;
	counter=0;
	   ...
	   case 1:  //PQ bus
		 // Get the values in the Jaccobian matrix
			for (j=ep[i]; j<ep[i+1]; ++j){
				  ...
				  std::cout << "J: i= " << i << ", j= " << j << ", p= " << p << ", H, " << H[j] << ", N, " << N[j] << ", J, " << J[j] << ", L, " << L[j] << std::endl;
			}
		 break;
	   case 2:  //PV bus
		 // deltaQ[i]=0; 
	...
	GraphLU_ResetMatrixValues(graphlu, Jx);
		if (!ret)
			...
			GraphLU_Factorize(graphlu);  
	GraphLU_Solve(graphlu, X_operation); // This is full forward/backward, the LU factorization of Jx is included in "graphlu"
	
	for (i=0; i<n; ++i){
		P_operation[i] = X_operation[2*i];
		Q_operation[i] = X_operation[2*i+1];

		dVa_sens[i] += P_operation[i];	// Calculate delta_Va/delta_lambda
		dVm_sens[i] += Q_operation[i];	// Calculate delta_Vm/delta_lambda
		
		if (abs(dVm_sens[i]-0)>0.0001){
			Zth=sqrt((dVm_sens[i]/Vm[i])*(dVm_sens[i]/Vm[i])+dVa_sens[i]*dVa_sens[i]);
			ZL=sqrt((1-dVm_sens[i]/Vm[i])*(1-dVm_sens[i]/Vm[i])+dVa_sens[i]*dVa_sens[i]);
			STI[i]=Zth[i]/ZL[i];
		}
				
		std::cout << "Vm:" << Vm[i] << " Pn:" << Pn[i] << " Qn:" << Qn[i] << " dVa_sens:" << dVa_sens[i] << " dVm_sens:" << dVm_sens[i] << " STI:" << STI[i] << std::endl;
	}
...
// **********************************************************************************************************************************
template <typename T_vertex_tuple, typename vertex_comp, typename T_edge_all_tuple, typename edge_all_comp>
inline string GRAPHLU_CG_powerflow_v7 (int64_t& gBp, int64_t& gBpp, HeapAccum<T_vertex_tuple, vertex_comp>& gVertex, HeapAccum<T_edge_all_tuple, edge_all_comp>& gEdge_all, double maxchange_ex, int ExMaxIteration, double maxchange_in, int InMaxIteration) {


	// extern size_t g_si, g_sd, g_sp;
	printf("\n\n------------------------------------------------------------------------------------------- ");
	printf("\nStart Running GRAPHLU_CG_powerflow_v7 C function!\n");
  std::cout << "Bp Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gBp << std::endl;
  std::cout << "Bpp Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gBpp << std::endl;
  std::cout << "Y bus Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gEdge_all.data_.size() << std::endl;
	printf("-------------------------------------------------------------------------------------------- \n\n");	
	...
	// Initialize arrays and variables
	uint__t n, nnz, n_pp, nnz_pp, n_e, nnz_e;
	int ret, i, j, p, iter_ex, iter_in, maxiter_ex=ExMaxIteration, maxiter_in=InMaxIteration;
	real__t maxDeltaP=0, maxDeltaQ=0, max_change_ex=maxchange_ex, max_change_in=maxchange_in;
	SGraphLU *graphlu, *graphlu_pp;
	...
	//// Convert vectors to GRAPHLU Data Structure (pointers)
	real__t *ax, *ax_pp, *ax_diag, *ax_diag_pp,*eG, *eB, *deltaP, *deltaQ, *Vm, *Va, *Pn, *Qn; 
	uint__t *ai, *ai_pp, *ei, *piv, *btype; 
	uint__t *ap, *ap_pp, *ep; 
  ...
  // values are computed during Jacobian initialization and each iteration 
	...
	ai = (uint__t *)malloc(sizeof(uint__t)*(nnz)); // column indices of B'
	ap = (uint__t *)malloc(sizeof(uint__t)*(n+1)); // initial row pointers
	ax_diag = (real__t *)malloc(sizeof(real__t)*(n));

	ax_pp = (real__t *)malloc(sizeof(real__t)*(nnz_pp)); // values in B"
	ai_pp = (uint__t *)malloc(sizeof(uint__t)*(nnz_pp)); // column indices of B"
	ap_pp = (uint__t *)malloc(sizeof(uint__t)*(n_pp+1)); // initial row pointers
	ax_diag_pp = (real__t *)malloc(sizeof(real__t)*(n_pp));
	...
	// real__t max_change_inner = max_change_in;
  	// define the CG value
	real__t *residual,*old_residual, *directionP, *old_directionP, *z, *old_z, *tempAP;
	real__t alpha, beta, r_square, old_r_square, PAP, max_residual;
	
	max_residual = 0;
	
	residual = (real__t *)malloc(sizeof(real__t)*(n));
	old_residual = (real__t *)malloc(sizeof(real__t)*(n));
	directionP = (real__t *)malloc(sizeof(real__t)*(n));
	old_directionP = (real__t *)malloc(sizeof(real__t)*(n));
	z = (real__t *)malloc(sizeof(real__t)*(n));
	old_z = (real__t *)malloc(sizeof(real__t)*(n));
	tempAP = (real__t *)malloc(sizeof(real__t)*(n));
	
	 memset(residual, 0, sizeof(real__t)*(n));
	 memset(old_residual, 0, sizeof(real__t)*(n));
	 memset(directionP, 0, sizeof(real__t)*(n));
	 memset(old_directionP, 0, sizeof(real__t)*(n));
	 memset(z, 0, sizeof(real__t)*(n));
	 memset(old_z, 0, sizeof(real__t)*(n));
	 memset(tempAP, 0, sizeof(real__t)*(n));
  
	std::cout << " ======================== Initialization of ararys used to store the factorized matrix and LU ========================"<< std::endl;

	struct timeval t2_st, t2_end, t3_st, t3_end, t1_st, t1_end, t4_st, t4_end; long seconds, useconds;
  ...
  for (ii=0; ii<gEdge_all.size(); ++ii) {
    ...
    if(gEdge_all.data_[ii].Bpp_x != 0)
    {
	 ...
	 for(int i =0;i<n; i++ ){
		 for( int j =ap_pp[i];j<ap_pp[i+1];j++){
			  p =ai_pp[j];
			  if (i == p){
				  ax_diag_pp[i]=ax_pp[j];
			  }
		 }
	 }
  ...
  // Done converting all input HeapAccums to arrays
    ...
    //continue to do the iterations only if there is no error from factorization
  	...
  	for (iter_ex=0; iter_ex<maxiter_ex; ++iter_ex){
  		// update deltaP and deltaQ
  		maxDeltaP=0;
  		maxDeltaQ=0;
      ...
      gettimeofday(&t3_st, 0);
  		...
  		for (i=0; i<n; ++i){
            ...
            // }    
  			...
  			// check output
 			// std::cout << "Iter_ex " << iter_ex<< ", deltaP" << i << ", " << deltaP[i]<< ", deltaQ" << i << ", " << deltaQ[i]<< std::endl;
  		}
  
  		std::cout << "Iter_ex " << iter_ex << " Updated for Va, maxDeltaP, " << maxDeltaP << ", maxDeltaQ, " << maxDeltaQ << std::endl;
   	  
      gettimeofday(&t3_end, 0);
  	  seconds=t3_end.tv_sec  - t3_st.tv_sec; 
  	  useconds = t3_end.tv_usec  - t3_st.tv_usec;
  	  // printf("\n\n==============================================================================================\n ");
  	  std::cout << "Time for updating deltaP and deltaQ (iteration):: " << iter_ex << " :" << (seconds*1000000 + useconds) << " us" << std::endl;
  	  // printf("\n================================================================================================\n ");
 ...
 // std::cout << " ======================== initialize CG for Va solve ========================"<<std::endl;
      gettimeofday(&t2_st, 0); 
  		// Calculate deltaP and deltaQ to update Va
		    memset(residual, 0, sizeof(real__t)*(n));
	        memset(old_residual, 0, sizeof(real__t)*(n));
  		...
  		for (i=0; i<n; ++i){
			...
			// if (btype[i]!=3){
		      // for (j=ap[i]; j<ap[i+1]; ++j){
  		      ...
  		      // }
		    // }
	        // std::cout << " deltaP" << i << ", " << deltaP[i] << ", deltaPn "<< b[i] << std::endl;
				
			 // initialize CG iteration	
			residual[i] = deltaP[i];
			old_residual[i] = residual[i];
			z[i] = residual[i]/ax_diag[i];			
			old_z[i] = z[i];
			directionP[i] = z[i]; 
			old_directionP[i] = directionP[i];	
			...
			// Get max residual
  			...
  			// check output
 			// std::cout << " CG-updated residual " << rp[i] << ", " << residual[i]<<  std::endl;
 		}
		gettimeofday(&t3_end, 0);
  	     seconds=t3_end.tv_sec  - t3_st.tv_sec; 
  	     useconds = t3_end.tv_usec  - t3_st.tv_usec;
  	     std::cout << "Time for ExprFunction-CG: CG init: consuming " << (seconds*1000000 + useconds) << " us, max_residual: " << max_residual << std::endl;
	  ...
	  memset(deltaPhi, 0, sizeof(double)*(n));
  	  ...
  	  // std::cout << " ======================== ExprFunction-CG Solver for V angle ========================"<<std::endl;
  	  for (iter_in=0; iter_in<maxiter_in; ++iter_in)   // while( max_residual > max_change_in && iter_in < maxiter_in )
	  {
		  // std::cout << " ======================== CG Solver for V angle ======================== iter_in: "<< iter_in <<std::endl;
		  // iter_in++;
		  max_residual =0;
		  gettimeofday(&t4_st, 0);
		  // update tempAP and PAP
		for (i=0; i<n; i++)
		 {
			 for (j=ap[i];j<ap[i+1]; j++)
			 {
				 ...
				 if (btype[i]!=3)
			     { 
			       tempAP[i] += ax[j]*old_directionP[p];  
				 }
				 // PAP += old_directionP[i] * tempAP[i];
				 // std::cout << " Debug tempAP: from:"<< i << ", to:" <<p<< ", tempAP="<< tempAP[i]<<", old_directionP[i]=" <<old_directionP[i]<<", ax="<< ax[j] << ", old_directionP[p]=" <<old_directionP[p] <<std::endl;
			 }
			 PAP += old_directionP[i] * tempAP[i];
			 old_r_square += old_residual[i]*old_z[i];	     
			 // std::cout << " Before Va CG ======= tempAP["<< i << "] =" << tempAP[i] <<std::endl;
	     }
		 
		 // calculate alpha
		 alpha = old_r_square/PAP;
		 // std::cout << " Debug 1 ======= alpha = "<< alpha << ", old_r_square =" << old_r_square <<", PAP =" << PAP <<std::endl;
		 gettimeofday(&t4_end, 0);
  	     seconds=t4_end.tv_sec  - t4_st.tv_sec; 
  	     useconds = t4_end.tv_usec  - t4_st.tv_usec;
  	     // std::cout << "Time for ExprFunction-CG: edge-loop for alpha: "<< iter_in << "th, consuming " << (seconds*1000000 + useconds) << " us" << std::endl;
		 
		 //update deltaPhi and residual
		 gettimeofday(&t4_st, 0);
		 for (i=0; i<n; i++)
		 {
			 //xk = xk-1 + alpha * Pk-1
			 deltaPhi[i] += alpha * old_directionP[i];
			 //rk = rk-1 - alpha*A*pk-1
			 residual[i] = old_residual[i] - alpha * tempAP[i];
			 z[i] = residual[i]/ax_diag[i];
			 r_square += residual[i]*z[i];
			 // std::cout << " residual["<< i << "]=" << residual[i] <<" deltaPhi[i]=" << deltaPhi[i] <<" tempAP[i]=" << tempAP[i] <<"z[i] ="<<z[i]<<std::endl;
			 
			 if (abs(residual[i]) > max_residual && btype[i]!=3)
  				max_residual = abs(residual[i]);
			
			 old_residual[i] = residual[i];
			 old_z[i] = z[i];
		 }
		 // std::cout << "==================== ExprFunction-CG: CG_Va iter: " << iter_in << ", max_residual: " << max_residual << std::endl;
		if ( max_residual < max_change_in){
        	// update Va  &&  reset for next ex-iter
             for (i=0;i<n;i++)
		     {
			    Va[i] -= deltaPhi[i];
	            // std::cout <<  i << " DCG Solved Va[i], " << Va[i] << std::endl;
		     }
			memset(deltaPhi, 0, sizeof(double)*(n));
		 ...
		 // update beta
		 beta = r_square/ old_r_square;
		 // std::cout << " Debug 1 ======= beta = "<< beta << ", r_square =" << r_square <<", old_r_square =" << old_r_square <<std::endl;
		 gettimeofday(&t4_end, 0);
  	     seconds=t4_end.tv_sec  - t4_st.tv_sec; 
  	     useconds = t4_end.tv_usec  - t4_st.tv_usec;
  	     // std::cout << "Time for ExprFunction-CG: node-loop1 for beta: "<< iter_in << "th, consuming " << (seconds*1000000 + useconds) << " us" << std::endl;
		 ...
		 for (i=0; i<n; i++)
		 {
			 directionP[i] = z[i] + beta * old_directionP[i];
			 old_directionP[i] = directionP[i];	
			 // std::cout << " Debug directionP:  directionP["<< i << "]=" << directionP[i] <<", z[i]=" << z[i] <<std::endl;
			 // tempAP[i]=0;
		 }
         gettimeofday(&t4_end, 0);
  	     seconds=t4_end.tv_sec  - t4_st.tv_sec; 
  	     useconds = t4_end.tv_usec  - t4_st.tv_usec;
  	     // std::cout << "Time for ExprFunction-CG: node-loop2 to update direction: "<< iter_in << "th, consuming " << (seconds*1000000 + useconds) << " us" << std::endl;		 
	  }   //End of inner iteration
	  
	  gettimeofday(&t3_end, 0);
  	  seconds=t3_end.tv_sec  - t3_st.tv_sec; 
  	  useconds = t3_end.tv_usec  - t3_st.tv_usec;
  	  std::cout << "Time for ExprFunction-CG: total iter_ins: "<< iter_in << ", consuming " << (seconds*1000000 + useconds) << " us" << std::endl;


	gettimeofday(&t2_end, 0);
 	seconds=t2_end.tv_sec  - t2_st.tv_sec; 
 	useconds = t2_end.tv_usec  - t2_st.tv_usec;
    // std::cout << "=====Time for ExprFunction-CG solve Va:"  << (seconds*1000000 + useconds) << " us" << std::endl;
      ...
      gettimeofday(&t3_st, 0);
  		...
  		std::cout << "Iter_ex" << iter_ex << " Updated for Vm, maxDeltaP, " << maxDeltaP << ", maxDeltaQ, " << maxDeltaQ << std::endl;
  ...
  // std::cout << " ======================== Update deltaQ and Initialize CG to solve Vm========================"<<std::endl;
      gettimeofday(&t2_st, 0); 
	    max_residual =0;
		memset(residual, 0, sizeof(real__t)*(n));
	    memset(old_residual, 0, sizeof(real__t)*(n));
		...
		gettimeofday(&t3_st, 0);
		  ...
		  if (btype[i]<2){
		    for (j=ap_pp[i]; j<ap_pp[i+1]; ++j){
			 ...
			 // initialize CG iteration	
			// residual[i] = deltaQ[i]-b[cpi_pp[i]];
			residual[i] = deltaQ[i];
			old_residual[i] = residual[i];
			z[i] = residual[i]/ax_diag_pp[i];			
			old_z[i] = z[i];
			directionP[i] = z[i]; 
			old_directionP[i] = directionP[i];	
			...
			// Get max residual
  			...
  			// check output
 			// std::cout << " CG-updated residual " << i << ", " << residual[i]<< ", deltaQ " << deltaQ[i] << ", deltaQn "<< b[i] <<  std::endl;
 		}
		 gettimeofday(&t3_end, 0);
  	     seconds=t3_end.tv_sec  - t3_st.tv_sec; 
  	     useconds = t3_end.tv_usec  - t3_st.tv_usec;
	  ...
	  memset(deltaV, 0, sizeof(double)*(n));
  	  ...
  	  // std::cout << " ======================== ExprFunction-CG Solver for V magnitude ========================"<<std::endl;
  	  for (iter_in=0; iter_in<maxiter_in; ++iter_in)  //while( max_residual > max_change_in && iter_in < maxiter_in )
	  {
		   // std::cout << " ======================== CG Solver for V magnitude ======================== iter_in: "<< iter_in <<std::endl;
		  // reset 0
		  max_residual =0;
		  // iter_in++;
		  gettimeofday(&t4_st, 0);
		...
		for (i=0; i<n; i++)
		 {
			 for (j=ap_pp[i];j<ap_pp[i+1]; j++)
			 {
				 ...
				 if (btype[i]<2)
			     { 
			       tempAP[i] += ax_pp[j]*old_directionP[p];  
				 }
				 // PAP += old_directionP[i] * tempAP[i];
				 // std::cout << " Debug tempAP: from:"<< i << ", to:" <<p<< ", tempAP="<< tempAP[i]<<", old_directionP[i]=" <<old_directionP[i]<<", ax="<< ax[j] << ", old_directionP[p]=" <<old_directionP[p] <<std::endl;
			 }
		 ...
		 //update deltaV and residual
		 gettimeofday(&t4_st, 0);
		 for (i=0; i<n; i++)
		 {
			 //xk = xk-1 + alpha * Pk-1
			 deltaV[i] += alpha * old_directionP[i];
			 //rk = rk-1 - alpha*A*pk-1
			 residual[i] = old_residual[i] - alpha * tempAP[i];
		     z[i] = residual[i]/ax_diag_pp[i];
			 r_square += residual[i]*z[i];
			 // std::cout << " residual["<< i << "]=" << residual[i] <<" deltaV[i]=" << deltaV[i] <<" tempAP[i]=" << tempAP[i] <<"z[i] ="<<z[i]<<std::endl;
			 
			 if (abs(residual[i]) > max_residual && btype[i]<2)
  				max_residual = abs(residual[i]);
			
			 old_residual[i] = residual[i];
			 old_z[i] = z[i];
		 }
		 // std::cout << "=============== ExprFunction-CG: CG_Vm iter: " << iter_in << ", max_residual: " << max_residual << std::endl;
		 ...
		 if ( max_residual < max_change_in){
             // update Va  &&  reset for next ex-iter
		       ...
		       {
			      Vm[i] -= deltaV[i];
	              // std::cout <<  i << " DCG Solved Vm[i], " << Vm[i] << std::endl;
		       }
			   memset(deltaV, 0, sizeof(double)*(n));
  			break;
  		}
	...
	gettimeofday(&t2_end, 0);
 	seconds=t2_end.tv_sec  - t2_st.tv_sec; 
 	useconds = t2_end.tv_usec  - t2_st.tv_usec;
    // std::cout << "=====Time for ExprFunction-CG to solve Vm:"  << (seconds*1000000 + useconds) << " us" << std::endl;
	// printf("\n================================================================================================\n ");
  	...
  	for (ii=0; ii<n; ++ii) {
        gVertex.data_[ii].Va = Va[ii]; ///pi_value*180;
        gVertex.data_[ii].Vm = Vm[ii];
        // std::cout << "Final, " << ii+1 << ", Vm, " << Vm[ii]<< ", Va" << ", " << Va[ii]<< std::endl;
  	 }
...
EXIT:
    ...
    printf("\n\n----------------------------------------------------------------------------------------");
	printf("\t\t End of Running GRAPHLU_CG_powerflow_v7 C function!");
	printf("\t\t ----------------------------------------------------------------------------------------\n\n");	
...
// **********************************************************************************************************************************
template <typename T_vertex_tuple, typename vertex_comp, typename T_edge_all_tuple, typename edge_all_comp>
inline string GRAPHLU_ILUPCG_powerflow_v7 (int64_t& gBp, int64_t& gBpp, HeapAccum<T_vertex_tuple, vertex_comp>& gVertex, HeapAccum<T_edge_all_tuple, edge_all_comp>& gEdge_all, double maxchange_ex, int ExMaxIteration, double maxchange_in, int InMaxIteration, double ILU_threshold_Bp, double ILU_threshold_Bpp) {


	// extern size_t g_si, g_sd, g_sp;
	printf("\n\n------------------------------------------------------------------------------------------- ");
	printf("\nStart Running GRAPHLU_ILUPCG_powerflow_v7 C function!\n");
  std::cout << "Bp Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gBp << std::endl;
  std::cout << "Bpp Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gBpp << std::endl;
  std::cout << "Y bus Number of rows:" << gVertex.data_.size() << ",\tNumber of nonzeros:" << gEdge_all.data_.size() << std::endl;
	printf("-------------------------------------------------------------------------------------------- \n\n");	
  ...
  // values are computed during Jacobian initialization and each iteration 
	...
	ap = (uint__t *)malloc(sizeof(uint__t)*(n+1)); // initial row pointers
	ax_diag = (real__t *)malloc(sizeof(real__t)*(n));
	// ax_org = (real__t *)malloc(sizeof(real__t)*(nnz)); // values in B' 
	// ai_org = (uint__t *)malloc(sizeof(uint__t)*(nnz)); // column indices of B'
	// ap_org = (uint__t *)malloc(sizeof(uint__t)*(n+1)); // initial row pointers

	ax_pp = (real__t *)malloc(sizeof(real__t)*(nnz_pp)); // values in B"
	ai_pp = (uint__t *)malloc(sizeof(uint__t)*(nnz_pp)); // column indices of B"
	ap_pp = (uint__t *)malloc(sizeof(uint__t)*(n_pp+1)); // initial row pointers
	ax_diag_pp = (real__t *)malloc(sizeof(real__t)*(n_pp));
	// ax_pp_org = (real__t *)malloc(sizeof(real__t)*(nnz_pp)); // values in B"
	// ai_pp_org = (uint__t *)malloc(sizeof(uint__t)*(nnz_pp)); // column indices of B"
	// ap_pp_org = (uint__t *)malloc(sizeof(uint__t)*(n_pp+1)); // initial row pointers
  ...
  for (ii=0; ii<gEdge_all.size(); ++ii) {
   ...
   // std::cout << " ======================== Get original Bp Factor ========================"<<std::endl;	
	// for (i=0; i<nnz; ++i){
     // std::cout<< i << ",Bx,"<< ax[i] << ",Bi,"<< ai[i] << std::endl; 
  // }
 
  // for (i=0; i<n; ++i){
     // std::cout<< i << ", Bp_p:" << ap[i] <<std::endl;
  // }
  ...
  // [Ting][end] comment out if to try sequential factorization 
	...
	// 								Filter the Lpp and Upp from Bpp Matirx
	// ----------------------------------------------------------------------------------------------------
	// download the GRAPHLU factorized data
	 real__t *lx, *ux; 
     uint__t *li, *ui; 
     size_t *lp, *up; 
     uint__t fctnnz; // number of nonzeros after the matrix is factorized
  ...
  pivot_inv = graphlu->pivot_inv; // pivoting array   
   ...
   // std::cout << " ======================== Get permuted Bp Factor ========================"<<std::endl;	
	// for (i=0; i<nnz; ++i){
     // std::cout<< i << ",Bx,"<< ax[i] << ",Bi,"<< ai[i] << std::endl; 
  // }
 
  // for (i=0; i<n; ++i){
     // std::cout<< i << ",ap,"<< ap[i+1] << ",rp,"<< rp[i] <<",cpi,"<< cpi[i] << ", Bp_p:" << ap[i] <<std::endl;
  // }
   ...
   for (int i =0;i<n;i++)
	 {
		 ...
		 for (j = lstart;j<lend;j++)
		 {
			 ...
			 int lindex = (lrow+1)*100000+lcol+1; 
			 lfillins_flag[j]=Bp_nofillins.count(lindex);
			 // std::cout << " L_fillin_flag ["<< j << "] = "<< lfillins_flag[j]  << ", Lx: "<< lx[j]<< std::endl;
			 // std::cout << " L_connection, "<< j <<", lrow: "<< cpi[lrow] <<  ", col: "<< cpi[lcol] << ", lindex: "<< lindex << std::endl;
		 }
	 }
	    ...
	    for (int i =0;i<n;i++)
	 {
		 ...
		 for (j = ustart;j<uend;j++)
		 {
			 ...
			 int uindex = (urow+1)*100000+ucol+1; 
			 ufillins_flag[j]=Bp_nofillins.count(uindex);
			 // std::cout << " U_fillin_flag ["<< j << "] = "<< ufillins_flag[j] <<", Ux: "<< ux[j]<<  std::endl;
		 }
	 }
	 ...
	 for (int i = 0; i<n; i ++)
	 {
		 ...
		 for (j = lstart;j<lend;j++)
		 {
			 ...
			 double templx = lx[j]/ldiag[lcol];  // LDLT
			 // std::cout << " LDLT: temp Lx["<<j<<"] = "<< templx <<std::endl;
		 if ((lfillins_flag[j] == 0) && (abs(templx))<= ILU_threshold_Bp )
		   {
				 lx[j]=0;
				 ilu_l_nnz--;
				 // std::cout << "Lx["<<j<<"] = "<< lx[j]<<std::endl;
	 	 ...
	 	 for (int i = 0; i<u_nnz; i ++)
	 {
		 if ((ufillins_flag[i] == 0) && (abs(ux[i]))<= ILU_threshold_Bp )
		   {
				 ux[i]=0;
				 ilu_u_nnz--;
				 // std::cout << "Ux["<<i<<"] = "<< ux[i]<<std::endl;
 	...
 	std::cout << " ======================== Number of nonzeros in L: "<< graphlu->l_nnz << "========================"<<std::endl;
 	std::cout << " ======================== Number of nonzeros in U: "<< graphlu->u_nnz << "========================"<<std::endl;
	std::cout << " ======================== Number of filtered nonzeros in L/U: "<< l_nnz-ilu_l_nnz << "========================"<<std::endl;
	// std::cout << " ======================== Number of nonzeros in ILU filtered L: "<< ilu_l_nnz << "========================"<<std::endl;
 	// std::cout << " ======================== Number of nonzeros in ILU filtered U: "<< ilu_u_nnz << "========================"<<std::endl;
	//std::cout << " ======================== End of LU Factorization Routines ========================"<<std::endl;
   ...
   for (int i =0;i<n;i++)
	 {
		 ...
		 for (j = lstart;j<lend;j++)
		 {
			 ...
			 //  std::cout << " L matrix ====== lindex = " << lindex << " Lx = "<< lx[j] << std::endl;
			 lfillins_flag_pp[j]=Bpp_nofillins.count(lindex);
			  // std::cout << " L_fillin_flag_pp ["<< j << "] = "<< lfillins_flag_pp[j] << " Lx = "<< lx_pp[j] << std::endl;
		 }
	 }
	    ...
	    for (int i =0;i<n;i++)
	 {
		 ...
		 for (j = ustart;j<uend;j++)
		 {
			 ...
			 // std::cout << " U matgrix ====== uindex = " << uindex  << " Ux = "<< ux[j] << std::endl;
			 ufillins_flag_pp[j]=Bpp_nofillins.count(uindex);
			 // std::cout << " U_fillin_flag_pp ["<< j << "] = "<< ufillins_flag_pp[j] << " Ux_pp = "<< ux_pp[j]  << std::endl;
		 }
	 }
	 ...
	 for (int i = 0; i<n; i++)
	 {
		 ...
		 for (j = lstart;j<lend;j++)
		 {
			 ...
			 double templx_pp = lx_pp[j]/ldiag_pp[lcol];  // LDLT
			 // std::cout << " LDLT: temp Lx["<<j<<"] = "<< templx <<std::endl;
		 if ((lfillins_flag_pp[j] == 0) && (abs(templx_pp))<= ILU_threshold_Bpp )
		   {
				 lx_pp[j]=0;
				 ilu_lpp_nnz--;
				 // std::cout << "Lx["<<j<<"] = "<< lx_pp[j]<<std::endl;
			 }
			// std::cout << "filtered Lx["<<j<<"] = "<< lx_pp[j]<<std::endl;
		 }
	 }
	 	 for (int i = 0; i<u_nnz_pp; i++)
	 {
		 if ((ufillins_flag_pp[i] == 0) && (abs(ux_pp[i]))<= ILU_threshold_Bpp )
		   {
				 ux_pp[i]=0;
				 ilu_upp_nnz--;
				 // std::cout << "Ux["<<i<<"] = "<< ux_pp[i]<<std::endl;
			 }
			  // std::cout << "filtered Ux["<<i<<"] = "<< ux_pp[i]<<std::endl;
	 } 
	 
	std::cout << " ======================== Number of Total nonzeros after factorization is: "<< graphlu_pp->lu_nnz << "========================"<<std::endl;
 	std::cout << " ======================== Number of nonzeros in L: "<< graphlu_pp->l_nnz << "========================"<<std::endl;
 	std::cout << " ======================== Number of nonzeros in U: "<< graphlu_pp->u_nnz << "========================"<<std::endl;
	std::cout << " ======================== Number of filtered nonzeros in L/U: "<< l_nnz_pp-ilu_lpp_nnz << "========================"<<std::endl;
	// std::cout << " ======================== Number of nonzeros in ILU filtered L: "<< ilu_lpp_nnz << "========================"<<std::endl;
 	// std::cout << " ======================== Number of nonzeros in ILU filtered U: "<< ilu_upp_nnz << "========================"<<std::endl;	 
  ...
  if(error_p >= 0 && error_pp >= 0){
    //continue to do the iterations only if there is no error from factorization
      ...
      maxDeltaQ = 0;
  		...
  		// solve for deltaVa
   // std::cout << " ======================== ILU Solve for V angle ========================"<<std::endl;
    gettimeofday(&t3_st, 0);
  		    // A*x = b
    // multiply deltaP with row scaling (rows) to get b
   	for (i=0; i<n; ++i){
	...
	// Update V angle (Va)
      ...
      {               
          deltaPhi[i] = b[i]; // save b[i] to deltaphi calculate the deltaPn = B'*(deltaphi) 
		  Va[i] -= b[cpi[i]];
          // std::cout << "ILU Solved Va, " << i << ", " << Va[i] << std::endl; 		  
      }    
...
/* 	  // Calculate deltaPn to update delta Phi
	  ...
	  gettimeofday(&t3_end, 0);
  	  ...
  	  useconds = t3_end.tv_usec  - t3_st.tv_usec;
  	  // printf("\n\n==============================================================================================\n ");
  	  std::cout << "Time for ILU solve Va :: " << iter_ex << " :" << (seconds*1000000 + useconds) << " us" << std::endl;
  	  // printf("\n================================================================================================\n ");

 // std::cout << " ======================== Update deltaP and initialize CG for Va solve ========================"<<std::endl;
      gettimeofday(&t2_st, 0); 
  		// Calculate deltaP and deltaQ to update Va
		    memset(residual, 0, sizeof(real__t)*(n));
	        memset(old_residual, 0, sizeof(real__t)*(n));
  	  ...
  	  for (iter_in=0; iter_in<maxiter_in; ++iter_in)   // while( max_residual > max_change_in && iter_in < maxiter_in )
	  {
		...
		for (i=0; i<n; i++)
		 ...
		 for (i=0; i<n; i++)
		 {
			 ...
			 if (abs(residual[i]) > max_residual && btype[rp[i]]!=3)
  				max_residual = abs(residual[i]);
     ...
     // std::cout << "=====Time for ExprFunction-CG solve Va:"  << (seconds*1000000 + useconds) << " us" << std::endl;
      ...
      gettimeofday(&t3_end, 0);
        ...
        result = "OK";
  		...
  		// solve for deltaVm
  		// std::cout << " ======================== ILU Solve for V magnitude ========================"<<std::endl;
		gettimeofday(&t3_st, 0);
  		    // A*x = b
     // multiply deltaQ with row scaling (rows_pp) to get b
   	for (i=0; i<n; ++i){
     ...
     // Full backward substitution 
   	for (i=n-1; i>=0; --i)
   	{
  		...
  		for (i=0; i<n; ++i)
  		{
      	   Vm[i] -= b[cpi_pp[i]];
		   deltaV[i] = b[i]; 
           // std::cout << "ILU Solved Vm, " << i << ", " << Vm[i] << std::endl; 	
        }
  	  ...
  	  useconds = t3_end.tv_usec  - t3_st.tv_usec;
  	  // printf("\n\n==============================================================================================\n ");
  	  std::cout << "Time for ILU solve Vm :: " << iter_ex << " :" << (seconds*1000000 + useconds) << " us" << std::endl;
  	  // printf("\n================================================================================================\n ");
  	  ...
  	  for (iter_in=0; iter_in<maxiter_in; ++iter_in)  //while( max_residual > max_change_in && iter_in < maxiter_in )
	  {
		...
		for (i=0; i<n; i++)
		 ...
		 for (i=0; i<n; i++)
		 {
			 ...
			 if (abs(residual[i]) > max_residual && btype[rp_pp[i]]<2)
  				max_residual = abs(residual[i]);
...
EXIT:
	...
	free(lfillins_flag);
	free(ufillins_flag);
	free(lfillins_flag_pp);
	free(ufillins_flag_pp);
	
    printf("\n\n----------------------------------------------------------------------------------------");
	printf("\t\t End of Running GRAPHLU_ILUPCG_powerflow_v7 C function!");
	printf("\t\t ----------------------------------------------------------------------------------------\n\n");	
