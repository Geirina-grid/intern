...
void _I_GraphLU_Factorize_Cluster(SGraphLU *graphlu, SGraphLUThreadArg *tharg, unsigned int id)
{
	real__t *ax;
	uint__t *ai, *ap;
	...
	offd = 0;
	ldiag = graphlu->ldiag;
	lu = graphlu->lu_array2;
	llen = graphlu->llen;
	ulen = graphlu->ulen;
	p = graphlu->pivot;
	pinv = graphlu->pivot_inv;
	tol = graphlu->cfgf[0];
	state = graphlu->row_state;
	aeg = graphlu->aeg_data;
	...
	for (ii=start; ii<end; ++ii)
	{
		...
		/*symbolic*/
		top = _I_GraphLU_Symbolic_Cluster(lu, n, i, pinv, stack, flag, pend, appos, \
		...
		if (tl > len_est[i])
		{
			...
			if (ptr == NULL)
			{
				tharg->err = GRAPHLU_MEMORY_OVERFLOW;
		...
		/*pivoting*/
		diagcol = p[i];
		err = _I_GraphLU_Pivot_Parallel(diagcol, &ulen[i], tmpu, tol, x, \
			&pivcol, &pivot, lurow);
		if (FAIL(err))
		{
			tharg->err = err;
		...
		/*prune*/
		_I_GraphLU_Prune_Parallel(pend, ll, ulen, pinv, pivcol, lrow_index, lu);
...
void _I_GraphLU_Factorize_Pipeline(SGraphLU *graphlu, SGraphLUThreadArg *tharg, unsigned int id)
{
	real__t *ax;
	uint__t *ai, *ap;
	uint__t cstart, cend, oldrow;
	...
	updated = (int__t *)(graphlu->workspace_mt2[id]);
	busy = updated + n;
	pruned = busy + n;
	...
	for (ii=start+id; ii<end; ii+=threads)
	{
		...
		while (!(*wait))
		{
			...
			for (l=graphlu->last_busy; l<ii; ++l)
			{
				busy[aeg[l]] = chkflg;
			}
...
POST:
		...
		/*post-symbolic*/
		top = _I_GraphLU_Symbolic_Pipeline(lu, n, i, pinv, stack, flag, pend, appos, \
...
/*main scheduler*/
int GraphLU_Factorize_MT(SGraphLU *graphlu)
{
	uint__t n, i, j, t;
	int__t *p, *pinv;
	int__t *pend;
	...
	for (ti=0; ti<th; ++ti)
	{
		memset(graphlu->workspace_mt1[ti], 0, sizeof(real__t)*n);/*x*/
		memset(((real__t *)(graphlu->workspace_mt1[ti]))+n, 0xff, sizeof(int__t)*n);/*flag*/
		memset(graphlu->workspace_mt2[ti], 0xff, sizeof(int__t)*(n+n));/*updated and busy*/
		arg[ti].lnnz = 0;
		arg[ti].unnz = 0;
		arg[ti].offdiag = 0;
		arg[ti].err = GRAPH_OK;
	...
	while (clv < level)
	{
		...
		graphlu->thread_work = ((mode == 0) ? GRAPHLU_WORK_FACT_CLUSTER : GRAPHLU_WORK_FACT_PIPELINE);
		lstart = clv;
		...
		if (mode == 0)/*cluster*/
		{
			...
			while (lv < lend)
			{
				...
				_I_GraphLU_Factorize_Cluster(graphlu, arg, 0);
		...
		else/*pipeline*/
		{
			...
			graphlu->pipeline_start = aeg_head[lstart];
			graphlu->pipeline_end = aeg_head[lend];
			graphlu->last_busy = graphlu->pipeline_start;
			...
			_I_GraphLU_Factorize_Pipeline(graphlu, arg, 0);
	...
	graphlu->l_nnz = tl + n;
	graphlu->u_nnz = tu + n;
	graphlu->lu_nnz = nnz + n;
	graphlu->stat[14] = (real__t)offd;
	...
	if (nnz > graphlu->lu_nnz_est)
	{
		graphlu->lu_nnz_est = nnz;
	}
	if (lu == NULL)
	{
		lu = malloc(g_sp*(graphlu->lu_nnz_est));
		if (lu == NULL)
		{
	...
	{
		void *ptr = realloc(lu, g_sp*(graphlu->lu_nnz_est));
		if (ptr == NULL)
		{
			return GRAPHLU_MEMORY_OVERFLOW;
		}
		graphlu->lu_array = ptr;
	}
...
#else
	/*parallel*/
	graphlu->thread_work = GRAPHLU_WORK_COPY_DATA;
	...
	nnz = (size_t)(graphlu->lu_nnz/(real__t)th * bal);
