...
int__t _I_GraphLU_MC64ad(uint__t n, uint__t nnz, uint__t *ai, uint__t *ap, real__t *ax, \
	int__t *match, int__t *match2, uint__t liw, int__t *iw, uint__t ldw, real__t *dw)
{
	...
	rt_ct = _I_GraphLU_MC64wd(n, nnz, ai, ap, dw_abs, match, iw, &iw[n], &iw[2*n], &iw[3*n], &iw[4*n], dw, dd);
...
/****************************************************************************************************************/
int__t _I_GraphLU_MC64wd(uint__t n, uint__t nnz, uint__t *ai, uint__t *ap, real__t *ax, int__t *iperm, int__t *jperm, \
...
/*
		...
		for (kk=0; kk<q0; ++kk)
		{
			...
			/*	qlen++;*/
				l[col] = qlen++;
				_I_GraphLU_MC64dd(col, n, q, d__, l, c__2);
			}
			/*update tree*/
			jj = iperm[col];
			out[jj] = k;
		...
		for (jdum=0; jdum<num; ++jdum)
		{
			...
			if (low == up)
			{
				...
				/*qlen>=1*/
STEP4:
				_I_GraphLU_MC64ed(&qlen, n, q, d__, l, c__2);
				q[--low] = col;
				l[col] = low;
				if (qlen == 0)
				{
			...
			for (k=ap[j]; k<end; ++k)
			{
				...
				else
				{
					...
					if (dnew <= dmin)
					{
						if (l[col] >= 0)
						{
							_I_GraphLU_MC64fd(l[col], &qlen, n, q, d__, l, c__2);
						}
						q[--low] = col;
						l[col] = low;
					}
