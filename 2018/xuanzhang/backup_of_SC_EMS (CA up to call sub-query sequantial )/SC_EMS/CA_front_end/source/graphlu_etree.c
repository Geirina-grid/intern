...
int _I_GraphLU_CreateETree(SGraphLU *graphlu)
{
	...
	first = (int__t *)(graphlu->workspace);
	pp = first + n;
	root = pp + n;
	parent = root + n;
	...
	++level;
  printf("\n====== GRAPHLU: Etree level = %d", level ); 
	graphlu->aeg_level = level;

	data = graphlu->aeg_data;
	header = graphlu->aeg_header;
