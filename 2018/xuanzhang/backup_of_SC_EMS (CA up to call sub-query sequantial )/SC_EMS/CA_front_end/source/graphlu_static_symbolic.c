...
int _I_GraphLU_StaticSymbolicFactorize(SGraphLU *graphlu)
{
	...
	graphlu->stat[11] = (real__t)(used + n);

	memmul = graphlu->cfgf[1];
	if (memmul < 1.)
	{
		memmul = 1.;
		graphlu->cfgf[1] = memmul;
	}
	prow = graphlu->cfgi[4];
	if (prow == 0)
	{
	...
	graphlu->stat[10] = (real__t)flop;

	/*predict speedup*/
	end = (real__t *)graphlu->workspace;
	...
	else
	{
		...
		if (pflops == 0.) graphlu->stat[7] = (real__t)n;
		else graphlu->stat[7] = graphlu->stat[10] / pflops;
	}
	...
	if (pflops == 0.) graphlu->stat[8] = (real__t)n;
	else graphlu->stat[8] = graphlu->stat[10] / pflops;
