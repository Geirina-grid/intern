/*preprocess, including static pivoting and AMD ordering*/
/*last modified: june 11, 2013*/
/*author: Chen, Xiaoming*/

#include "graphlu.h"
#include "graphlu_internal.h"
#include "timer_c.h"
#include "math.h"

extern size_t g_sp;

int GraphLU_Analyze(SGraphLU *graphlu)
{
	int__t rank;
	int err;
	int__t *w;
	int__t *match, *match2;
	real__t *dw;
	uint__t n, nnz;
	real__t *ax;
	uint__t *ai, *ap;
	size_t size;

	uint__t *cp0;
	uint__t *ci0;
	uint__t *rp;
	uint__t *ri;
	uint__t *cp;
	uint__t *ci;
	int__t *len;
	uint__t nzaat;
	int__t *pe, *sp, *iw, *tp;
	int__t *p, *pinv;

	uint__t j;
	real__t t, *u, *v;

	real__t *rs, *csi;
	uint__t *cpi;
	uint__t i;
	uint__t *p1, *p2, *p3, *p4;

#ifdef GRAPHLU_DEBUG
	FILE *fp;
#endif

	if (NULL == graphlu)
	{
		return GRAPHLU_ARGUMENT_ERROR;
	}
	if (graphlu->flag[1]) return GRAPH_OK;
	if (!graphlu->flag[0])
	{
		return GRAPHLU_MATRIX_NOT_INITIALIZED;
	}

	/*begin*/
	TimerStart((STimer *)(graphlu->timer));

	/*set parameters*/
	rank = 0;
	err = GRAPH_OK;
	w = NULL;
	match = NULL;
	match2 = NULL;
	dw = NULL;
	n = graphlu->n;
	nnz = graphlu->nnz;
	ax = graphlu->ax;
	ai = graphlu->ai;
	ap = graphlu->ap;

	/*init*/
	p1 = graphlu->row_perm;
	p2 = graphlu->row_perm_inv;
	p3 = graphlu->col_perm;
	p4 = graphlu->col_perm_inv;
	rs = graphlu->row_scale;
	csi = graphlu->col_scale_perm;
	for (i=0; i<n; ++i)
	{
		p1[i] = p2[i] = p3[i] = p4[i] = i;
		rs[i] = csi[i] = 1.;
	}

	/**************************************************************************************/
	/*maximum matching*/
	size = n*5*sizeof(int__t);
	w = (int__t *)malloc(size);
	if (NULL == w)
	{
		return GRAPHLU_MEMORY_OVERFLOW;
	}

	size = sizeof(int__t) * (n+n);
	match = (int__t *)malloc(size);
	if (NULL == match)
	{
		free(w);
		return GRAPHLU_MEMORY_OVERFLOW;
	}
	match2 = match+n;

	/*workspace memory states
	 |-----|-----|-----|-----|-----|
	 flag(w) cheap  is   js    ps
	 w
	*/

	size = sizeof(real__t)*(3*n+nnz);
	dw = (real__t *)malloc(size);
	if (NULL == dw)
	{
		free(w);
		free(match);
		return GRAPHLU_MEMORY_OVERFLOW;
	}
	/*dw workspace
	      |-----|-----|-----|----------|
	       u     v     max   abs
		   */

	/*mc64*/
	rank = _I_GraphLU_MC64ad(n, nnz, ai, ap, ax, match, match2, 5*n, w, 3*n+nnz, dw);

	if (FAIL(rank))
	{
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return (int)rank;
	}

#ifdef GRAPHLU_DEBUG
	printf("final match: %d\n", rank);

	fp = fopen("match.txt", "w");
	for (i=0; i<n; ++i)
	{
		fprintf(fp, "match[%d] = %d\n", i, match[i]);
	}
	for (i=0; i<n; ++i)
	{
		fprintf(fp, "match2[%d] = %d\n", i, match2[i]);
	}
	fclose(fp);
#endif

	if ((uint__t)rank < n)/*not full rank*/
	{
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return GRAPHLU_MATRIX_STRUCTURAL_SINGULAR;
	}

	/**************************************************************************************/
	/*amd*/
	cp0 = NULL;
	ci0 = NULL;
	rp = NULL;
	ri = NULL;
	cp = NULL;
	ci = NULL;

	err = _I_GraphLU_Check(graphlu);

	if (FAIL(err))
	{
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return err;
	}

	size = sizeof(uint__t)*nnz;
	ci0 = (uint__t *)malloc(size);
	if (ci0 == NULL)
	{
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return GRAPHLU_MEMORY_OVERFLOW;
	}

	size = sizeof(uint__t)*(n+1);
	cp0 = (uint__t *)malloc(size);
	if (cp0 == NULL)
	{
		free(ci0);
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return GRAPHLU_MEMORY_OVERFLOW;
	}

	_I_GraphLU_ConstructCSR(graphlu, match, ci0, cp0);

	if (GRAPHLU_MATRIX_NOT_SORTED == err)/*need sort*/
	{
		size = sizeof(uint__t)*nnz;
		ri = (uint__t *)malloc(size);
		if (ri == NULL)
		{
			free(ci0);
			free(cp0);
			free(w);
			free(match);
			if (dw != NULL) free(dw);
			return GRAPHLU_MEMORY_OVERFLOW;
		}

		size = sizeof(uint__t)*(n+1);
		rp = (uint__t *)malloc(size);
		if (rp == NULL)
		{
			free(ci0);
			free(cp0);
			free(ri);
			free(w);
			free(match);
			if (dw != NULL) free(dw);
			return GRAPHLU_MEMORY_OVERFLOW;
		}

		_I_GraphLU_AMDSort(n, ci0, cp0, w, ri, rp);

		ci = ri;
		cp = rp;

		free(ci0);
		free(cp0);
	}
	else/*no sort*/
	{
		ci = ci0;
		cp = cp0;
	}

	/*a+a'*/
	size = sizeof(int__t)*n;
	len = (int__t *)malloc(size);
	if (NULL == len)
	{
		free(ci);
		free(cp);
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		return GRAPHLU_MEMORY_OVERFLOW;
	}

	/*the nonzeros in each row of a+a' (exclu diag)*/
	nzaat = _I_GraphLU_AAT(n, ci, cp, len, w);

	free(w);

	size = sizeof(int__t)*(((size_t)n)*7+nzaat+nzaat/5);
	w = (int__t *)malloc(size);
	if (NULL == w)
	{
		free(match);
		if (dw != NULL) free(dw);
		free(ci);
		free(cp);
		free(len);
		return GRAPHLU_MEMORY_OVERFLOW;
	}

	/*workspace memory states
	       |----|----|----|----|----|----|----|----------|
	       pe   nv   head elen degree w   iw
		   */

	pe = w;
	sp = w+n;
	iw = w+6*n;
	tp = w+5*n;

	/*construct A+A'*/
	_I_GraphLU_AAT2(n, ci, cp, len, pe, sp, iw, tp);

	free(ci);
	free(cp);

	size = sizeof(int__t) * (n+n);
	p = (int__t *)malloc(size);
	if (NULL == p)
	{
		free(w);
		free(match);
		if (dw != NULL) free(dw);
		free(len);
		return GRAPHLU_MEMORY_OVERFLOW;
	}
	pinv = p + n;

	_I_GraphLU_AMD(n, nzaat, n+nzaat+nzaat/5, pe, iw, len, w, p, pinv, \
		graphlu->cfgf[2], graphlu->cfgi[6], &(graphlu->lu_nnz_est));
	if (graphlu->lu_nnz_est < (size_t)nnz) graphlu->lu_nnz_est = nnz;

	free(len);

#ifdef GRAPHLU_DEBUG
	fp = fopen("permute.txt", "w");
	for (i=0; i<n; ++i)
	{
		fprintf(fp, "%d\t%d\t%d\n", i, p[i], pinv[i]);
	}
	fclose(fp);
	printf("lu  %u\n", graphlu->lu_nnz_est);
#endif

	_I_GraphLU_Permute(graphlu, match, p, pinv);

	free(match);
	free(p);
	free(w);

	/*mc64_scale*/
	u = dw;/*col*/
	v = u + n;/*row*/

	rs = graphlu->row_scale;
	csi = graphlu->col_scale_perm;
	cpi = graphlu->col_perm_inv;

	for (j=0; j<n; ++j)
	{
		t = exp(v[j]);
		rs[j] = t;

		t = exp(u[j]);
		csi[cpi[j]] = t;
	}

	_I_GraphLU_MC64Scale(graphlu);
	err = _I_GraphLU_Scale(graphlu);
	if (FAIL(err))
	{
		if (dw != NULL) free(dw);
		return err;
	}

	if (dw != NULL) free(dw);

#ifdef GRAPHLU_DEBUG
	fp = fopen("mc64_scale.txt", "w");
	for (i=0; i<n; ++i)
	{
		fprintf(fp, "%.16g  %.16g\n", graphlu->row_scale[i], graphlu->col_scale_perm[i]);
	}
	fclose(fp);
#endif

	/**************************************************************************************/
	/*finish*/
	graphlu->flag[1] = TRUE;

	TimerStop((STimer *)(graphlu->timer));
	graphlu->stat[0] = TimerGetRuntime((STimer *)(graphlu->timer));

	return GRAPH_OK;
}
