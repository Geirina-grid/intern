...
//	 double s = ax[j]*(Va[i] - Va[p])/20;
		 //std::cout<<"from: "<<i<<"to: "<<p << "s_value, " << s << "; line_Q1, "<<line_Q1[j]<< std::endl;
...
template <typename ca_heap_tuple, typename ca_heap_comp, typename pfRow>

inline void UniqueHeap (HeapAccum<ca_heap_tuple, ca_heap_comp>& ca_heap, SetAccum<pfRow>& pfSet, MaxAccum<int> max_line_num) {
	for (int i = 0; i <= ca_heap.data_.size(); i++){
		if (pfSet.data_.size() < max_line_num) {
			int s_id = ca_heap.data_[i].s_exId;
			int t_id = ca_heap.data_[i].t_exId;
			pfSet += pfRow(s_id, t_id);
		}
	}
}
