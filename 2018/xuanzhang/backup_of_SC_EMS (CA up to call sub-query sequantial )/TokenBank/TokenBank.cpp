...
//[begin] customized token functions
// _getG and _getB are N-tokens-in, one-token-out function to calculate G and B from R and X. N is two
extern "C" double _getG (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum)
{
  double R = atof(std::string(iToken[0],iTokenLen[0]).c_str());
  double X = atof(std::string(iToken[1],iTokenLen[1]).c_str());
  return R/(R*R + X*X);
}

extern "C" double _getB (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum)
{
  double R = atof(std::string(iToken[0],iTokenLen[0]).c_str());
  double X = atof(std::string(iToken[1],iTokenLen[1]).c_str());
  return X/(R*R + X*X);
...
extern "C" double _getCalP (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum)
{
  double GenP = atof(std::string(iToken[0],iTokenLen[0]).c_str());
  double LdP = atof(std::string(iToken[1],iTokenLen[1]).c_str());
  return GenP - LdP;
}

extern "C" double _getCalQ (const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum)
{
  double GenQ = atof(std::string(iToken[0],iTokenLen[0]).c_str());
  double LdQ = atof(std::string(iToken[1],iTokenLen[1]).c_str());
  return GenQ - LdQ;
}

// the functions Zero, divide and _minus used to be in TokenBank1.cpp prior to 4.4 since these
// are one-token-in, one-token-out functions. In 4.4 TokenBank1.cpp and TokenbankM.cpp are merged,
// and all functions should be N-Tokens-in, one-token-out, but N can be one or more 
extern "C"  uint64_t Zero(const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum) {
  return 0;
}

extern "C"  double divide(const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum) {
  return 1.0/atof(std::string(iToken[0], iTokenLen[0]).c_str());
}

extern "C"  double _minus(const char* const iToken[], uint32_t iTokenLen[], uint32_t iTokenNum) {
  return -1 * atof(std::string(iToken[0], iTokenLen[0]).c_str());
}
